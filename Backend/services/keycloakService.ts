...
export async function getKeycloakAdminToken(): Promise<string> {
  ...
  if (process.env.DESPLIEGUE === 'docker') {
    keycloakUrl = process.env.KEYCLOAK_URL_DOCKER!;
  } else {
    keycloakUrl = process.env.KEYCLOAK_URL_LOCAL!;
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(`Could not obtain Keycloak admin token: ${errorText}`);
  }
...
export async function createKeycloakUser(userData: CreateKeycloakUserData) {
  let keycloakUrl: string;
  ...
  if (!createUserResponse.ok) {
    ...
    throw new Error(`Could not create Keycloak user: ${errorText}`);
  }
  ...
  if (!location) {
    throw new Error('User was created but Keycloak did not return the user location');
  }

  const userId = location.split('/').pop();

  if (!userId) {
    throw new Error('Could not obtain the Keycloak user ID');
  }
  ...
  for (const roleName of userData.roles) {
    ...
    if (!roleResponse.ok) {
      throw new Error(`Role "${roleName}" does not exist in Keycloak`);
    }
    ...
    if (!assignRoleResponse.ok) {
      ...
      throw new Error(`Could not assign role "${roleName}": ${errorText}`);
    }
  }
